...
void con_obst(void);
void print(int,int);
float a[20],b[20],wt[20][20],c[20][20]; // a successful probability search 
                                        // b unsuccessful search w weight matrix c cost matrix
int r[20][20],n;
int main()
  {
	int i;
	...
	cout<<"\nEnter the probability for successful search :: "; //0.5 0.1 0.15
	cout<<"\n===================\n";
	for(i=1;i<=n;i++)
	  {
		cout<<"p["<<i<<"]";
	...
	cout<<"\nEnter the probability for unsuccessful search :: "; //0.15 0.1 0.05 0.05
	cout<<"\n====================\n";
	for(i=0;i<=n;i++)
	  {
		cout<<"q["<<i<<"]";
	...
	for(i=0;i<n;i++)  //n  no of nodes
	  {
	   //Initialisation
		...
		wt[i][i+1]=b[i]+b[i+1]+a[i+1];// wt0 1 b0 b1 a1
...
void print(int l1,int r1) //l1 child and r1 child
  {
	if(l1>=r1)
		return;
	if(r[l1][r[l1][r1]-1]!=0)
